...
function view_home_cat( $data_content, $compare_id, $html_pages = '', $sort = 0 )
{
	...
	if( !empty( $data_content ) )
	{
		foreach( $data_content as $data_row )
		{
			if( $data_row['num_pro'] > 0 )
			{
				//$data_row['title']=$data_content['image'].$data_row['title']; 
				$xtpl->assign( 'TITLE_CATALOG', $data_row['title'] );
				$xtpl->assign( 'LINK_CATALOG', $data_row['link'] );
				$xtpl->assign( 'NUM_PRO', $data_row['num_pro'] );
				//$xtpl->assign( 'SRC_ICATE', $data_row['image'] );
				...
				$image = NV_UPLOADS_REAL_DIR . '/' . $module_name . '/' . $data_row['image'];

				if( !empty( $data_row['image'] ) and file_exists( $image ) )
				{
					$xtpl->assign( 'IMAGE', NV_BASE_SITEURL . NV_UPLOADS_DIR . '/' . $module_name . '/' . $data_row['image'] );
...
function detail_product( $data_content, $data_unit, $data_shop, $data_others, $array_other_view, $content_comment, $compare_id, $popup )
{
	...
	$my_head .= "<link rel=\"Stylesheet\" href=\"" . NV_BASE_SITEURL . "js/slide/style_yahoo.css\" />\n";
	$my_head .= "<script type=\"text/javascript\" src=\"" . NV_BASE_SITEURL . "js/slide/jquery_002.js\"></script>\n";
	$my_head .= "<script type=\"text/javascript\" src=\"" . NV_BASE_SITEURL . "js/slide/script.js\"></script>\n";
	...
	if( !empty( $data_content ) )
	{
		...
		$xtpl->assign( 'SRC_DETAIL_INTRO', $data_content['detailintro'] );
		...
		if( !empty( $data_content['array_custom'] ) and !empty( $data_content['array_custom_lang'] ) )
		{
			...
			foreach( $data_content['array_custom'] as $field => $value )
			{
				if( $i > 2 )
				{
					$xtpl->assign( 'CUSTOM_DATA', $value[1] );
