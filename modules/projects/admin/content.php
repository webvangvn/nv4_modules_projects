...
$product_number = array(0=>array(0,$lang_module['warehouse_product_empty']),1 => array(1,$lang_module['warehouse_product_num']));
...
if( $nv_Request->get_int( 'save', 'post' ) == 1 )
{
	...
	$rowcontent['detailintro'] = $nv_Request->get_title( 'detailintro', 'post', '' );
	...
	if( empty( $error ) )
	{
		...
		if( !nv_is_url( $rowcontent['detailintro'] ) and is_file( NV_DOCUMENT_ROOT . $rowcontent['detailintro'] ) )
		{
			$lu = strlen( NV_BASE_SITEURL . NV_UPLOADS_DIR . '/' . $module_name . '/' );
			$rowcontent['detailintro'] = substr( $rowcontent['detailintro'], $lu );
		}
		else
		{
			$rowcontent['detailintro'] = '';
		}
		$listfield = '';
		$listvalue = '';
		...
		if( $rowcontent['id'] == 0 or $is_copy )
		{
			...
			$data_insert['detailintro'] = $rowcontent['detailintro'];
		...
		else
		{
			...
			$stmt->bindParam( ':detailintro', $rowcontent['detailintro'], PDO::PARAM_STR );
...
if( !empty( $rowcontent['detailintro'] ) and file_exists( NV_UPLOADS_REAL_DIR . '/' . $module_name . '/' . $rowcontent['detailintro'] ) )
{
	$rowcontent['detailintro'] = NV_BASE_SITEURL . NV_UPLOADS_DIR . '/' . $module_name . '/' . $rowcontent['detailintro'];
}
...
//con hang hay het hang


foreach( $product_number as $product_number_i )
	{
		if ($product_number_i[0]==$rowcontent['product_number']) { $ns="selected";} else {$ns="";}
		$data_product_number_i = array(
			'code' => $product_number_i[0],
			'select' => $ns,
			'title' => $product_number_i[1]
		);
		$xtpl->assign( 'NUM', $data_product_number_i );
